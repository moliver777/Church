class Panel < ActiveRecord::Base
  ...
  def build
    content = ActiveRecord::Base.connection.execute("SELECT SQL_NO_CACHE html_content FROM page_layouts WHERE id = #{self.page_layout_id}").first[0] rescue PageLayout.find(self.page_layout_id).html_content
    images = self.images
